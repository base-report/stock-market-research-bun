...
const addTrades = (setups: Setup[]) => {
  ...
  const insertRow = db.prepare(
    "INSERT INTO trades (code, prior_move_low_date, prior_move_high_date, prior_move_pct, consolidation_slope, consolidation_days, consolidation_start_date, consolidation_end_date, entry_price, entry_date, entry_trendline_break_price, entry_adr_pct, entry_dollar_volume, highest_price_date, highest_price, highest_price_days, exit_price, exit_date, exit_reason, exit_days, volatility_contraction, consolidation_quality) VALUES ($code, $prior_move_low_date, $prior_move_high_date, $prior_move_pct, $consolidation_slope, $consolidation_days, $consolidation_start_date, $consolidation_end_date, $entry_price, $entry_date, $entry_trendline_break_price, $entry_adr_pct, $entry_dollar_volume, $highest_price_date, $highest_price, $highest_price_days, $exit_price, $exit_date, $exit_reason, $exit_days, $volatility_contraction, $consolidation_quality) ON CONFLICT(id) DO UPDATE SET prior_move_low_date=excluded.prior_move_low_date, prior_move_high_date=excluded.prior_move_high_date, prior_move_pct=excluded.prior_move_pct, consolidation_slope=excluded.consolidation_slope, consolidation_days=excluded.consolidation_days, consolidation_start_date=excluded.consolidation_start_date, consolidation_end_date=excluded.consolidation_end_date, entry_price=excluded.entry_price, entry_date=excluded.entry_date, entry_trendline_break_price=excluded.entry_trendline_break_price, entry_adr_pct=excluded.entry_adr_pct, entry_dollar_volume=excluded.entry_dollar_volume, highest_price_date=excluded.highest_price_date, highest_price=excluded.highest_price, highest_price_days=excluded.highest_price_days, exit_price=excluded.exit_price, exit_date=excluded.exit_date, exit_reason=excluded.exit_reason, exit_days=excluded.exit_days, volatility_contraction=excluded.volatility_contraction, consolidation_quality=excluded.consolidation_quality"
  );
